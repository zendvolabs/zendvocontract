...
mod test {
    ...
    use crate::events::{FeeWithdrawal, WithdrawalSuccess};
    use crate::types::GiftStatus;
    use crate::TimeLockContract;
    use soroban_sdk::{
        testutils::{Address as TestAddress, Events, Ledger as TestLedger},
        BytesN, Env, String,
    };
    ...
    fn test_withdraw_gift_flow() {
        ...
        let withdrawal_event = events.iter().find(|e| {
            e.topics.len() > 0
                && e.topics.get(0).unwrap() == &soroban_sdk::symbol_short!("withdr_s")
        });
